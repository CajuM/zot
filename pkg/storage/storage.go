	...
	if config.Storage.StorageDriver == nil {
		...
		case constants.S3StorageDriverName:
			defaultStore = s3.NewImageStore(rootDir, config.Storage.RootDirectory,
				config.Storage.Dedupe, config.Storage.Commit, log, metrics, linter, store, cacheDriver, nil)
		case constants.AzureBlobStorageDriverName:
		...
		if storageConfig.StorageDriver == nil {
			...
			case constants.S3StorageDriverName:
				subImageStore[route] = s3.NewImageStore(rootDir, storageConfig.RootDirectory,
					storageConfig.Dedupe, storageConfig.Commit, log, metrics, linter, store, cacheDriver, nil,
				)
			case constants.AzureBlobStorageDriverName:
